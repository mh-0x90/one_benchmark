...
bp = Blueprint('tech_002_xs_s', __name__)

@bp.route('/sr/posts/<int:id>/comments', methods=['POST'])
def add_comment(id):
...
@bp.route('/sr/posts/<int:id>', methods=['GET'])
def view_post(id):
    post = Post.query.get_or_404(id)
    # ----
    return render_template('post.html', post=post)

@bp.route('/siterel/tech_002_xs_s', methods=['GET'])
def process_combined_data():
    initial_seed = 42.0
    string_seed = "QuantumLeapEngine"
    RANDOM_PATH_COMBINED = '/siterel/tech_002_xs_s'
